...
def sigusr1_handler(signum, frame):
	...
	print("fifo!")
	print(alist)
#	last_element = [int(x) for x in alist.split()][-1]
#	last_element = [(x) for x in alist.split()][-1]
#	last_element = [(x) for x in alist.split()][0]
	last_element = alist
	...
	try:
		bright = int(last_element)
		set_bright(1, bright)
		is_cur_bright_changed = 1
		print("is_cur : " + str(is_cur_bright_changed))
...
def set_req_light_bright(req_subject):
	global is_cur_bright_changed
	global is_person_here

	with open("light_bright_info.txt", 'r') as f:
		while True:
			subject_info = f.readline().split()
			if not subject_info: break

			if req_subject == subject_info[0]:
				print("set bright : " + str(subject_info[1]))
				print(int(subject_info[1]))

				if is_cur_bright_changed == 0:
					print("YEAH~")

					set_bright(1, int(subject_info[1]))
				return 0
			else:
				continue
	return -1
...
def handle_user_input():
	...
	signal.signal(signal.SIGUSR1, sigusr1_handler)
	signal.signal(signal.SIGINT, sigusr1_handler)
...
#	set_bright(1, 100)	
	while True:
		...
		already_exist = False
		prev_schedule = "0000000000"
		with open("schedule_info.txt", 'r') as f:
			while already_exist == False:
				...
				sched_datetime_str = schedule[0] + schedule[2]

				
				print(sched_datetime_str)
				print(now_datetime_str)
				print()

				time.sleep(1)
				if now_datetime_str < sched_datetime_str:
					if not prev_schedule == "0000000000":
						cur_subject = prev_schedule[1]
						print("prev : " + cur_subject)
						set_req_light_bright(cur_subject)
						already_exist = True
					else:
						cur_subject = 0
		...
		if already_exist == False:
			print(prev_schedule)
			cur_subject = prev_schedule[1]
			print("prev2 : " + cur_subject)
			if set_req_light_bright(cur_subject) != 0:
				print("set_req_light_bright error")
			
			
		time.sleep(10)


#		last_element = [int(x) for x in arrlist.split()]
